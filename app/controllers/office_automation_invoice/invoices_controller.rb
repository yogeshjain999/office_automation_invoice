...
module OfficeAutomationInvoice
  class InvoicesController < ApplicationController
    ...
    def create
      ...
      if params[:commit].present? # creating new invoice
      ...
      elsif params[:button].eql? 'preview'
        @invoice = Invoice.new(invoice_params)
        
        if @invoice.valid?
          @invoice.calculate_total
          respond_to do |format|
            format.js { render '/office_automation_invoice/invoices/preview' and return }
          end
        else
          flash[:notice] = 'Please fill the fields accordingly.'
        end
        ...
        @invoice = Invoice.new(invoice_params)
        @invoice.valid? ? @invoice.calculate_total : flash[:notice] = 'Please fill the fields accordingly.'
      end
    ...
    def invoice_params
      params.require(:invoice).permit(:issue_date, :due_date, :schedule_period, :schedule_type, :note, :discount, :total_amount, tax_ids: [], items_attributes: [:id, :srn, :description, :amount, :_destroy] )
    end
  end
end
